CREATE OR REPLACE PACKAGE BODY FORMULE_LYON3 AS
  ...
  BEGIN
    ...
    CASE c
      ...
      -- U=IF([.$I20]="Référentiel";0;([.$AJ20]+[.$BB20]))
      WHEN 'U' THEN
        IF vh.volume_horaire_ref_id IS NOT NULL THEN
          RETURN 0;
        ELSE
          RETURN (cell('AJ',l) + cell('BB',l));
        END IF;



      -- V=IF([.$I20]="Référentiel";0;[.$AP20])
      WHEN 'V' THEN
        IF vh.volume_horaire_ref_id IS NOT NULL THEN
          RETURN 0;
        ELSE
      ...
      -- Y=IF([.$I20]="Référentiel";0;IF([.$F20]<>0;([.$AL20]+([.$BD20]*[.$F20]));[.$AL20]))
      WHEN 'Y' THEN
        IF vh.volume_horaire_ref_id IS NOT NULL THEN
          RETURN 0;
        ELSE
          IF vh.taux_fi <> 0 THEN
            RETURN (cell('AL',l) + (cell('BD',l) * vh.taux_fi));
          ELSE
            RETURN cell('AL',l);
          END IF;
        END IF;
      ...
      -- Z=IF([.$I20]="Référentiel";0;IF([.$G20]<>0;[.$AR20]+([.$BD20]*[.$G20]);[.$AR20]))
      WHEN 'Z' THEN
        IF vh.volume_horaire_ref_id IS NOT NULL THEN
          RETURN 0;
        ELSE
          IF vh.taux_fa <> 0 THEN
            RETURN cell('AR',l) + (cell('BD',l) * vh.taux_fa);
          ELSE
            RETURN cell('AR',l);
          END IF;
        END IF;
      ...
      -- AA=IF([.$I20]="Référentiel";0;IF([.$H20]<>0;[.$AX20]+([.$BD20]*[.$H20]);[.$AX20]))
      WHEN 'AA' THEN
        IF vh.volume_horaire_ref_id IS NOT NULL THEN
          RETURN 0;
        ELSE
          IF vh.taux_fc <> 0 THEN
            RETURN cell('AX',l) + (cell('BD',l) * vh.taux_fc);
          ELSE
            RETURN cell('AX',l);
          END IF;
        END IF;
      ...
      -- AH=IF(AND([.$D20]<>"Oui";[.$F20]<>0;[.$I20]<>"Référentiel";[.$O20]="Oui");[.$N20]*[.$AE20]*[.$F20];0)
      WHEN 'AH' THEN
        IF NOT vh.structure_is_exterieur AND vh.taux_fi <> 0 AND vh.volume_horaire_ref_id IS NULL AND vh.param_1 = 'Oui' THEN
      ...
      -- AN=IF(AND([.$D20]<>"Oui";[.$G20]<>0;[.$I20]<>"Référentiel";[.$O20]="Oui");[.$N20]*[.$AE20]*[.$G20];0)
      WHEN 'AN' THEN
        IF NOT vh.structure_is_exterieur AND vh.taux_fa <> 0 AND vh.volume_horaire_ref_id IS NULL AND vh.param_1 = 'Oui' THEN
      ...
      -- AT=IF(AND([.$D20]<>"Oui";[.$H20]<>0;[.$I20]<>"Référentiel";[.$O20]="Oui";[.$E20]<>"Non");[.$N20]*[.$AE20]*[.$H20];0)
      WHEN 'AT' THEN
        IF NOT vh.structure_is_exterieur AND vh.taux_fc <> 0 AND vh.volume_horaire_ref_id IS NULL AND vh.param_1 = 'Oui' AND vh.service_statutaire THEN
      ...
      -- AZ=IF(AND([.$I20]<>"Référentiel";[.$O20]="Non");[.$N20]*[.$AE20];0)
      WHEN 'AZ' THEN
        IF vh.volume_horaire_ref_id IS NULL AND vh.param_1 = 'Non' THEN
  ...
  BEGIN
    ...
    SELECT
      fvh.*,
      case WHEN tf.diplome_national = 1 THEN ''Oui'' ELSE ''Non'' END param_1,
      NULL param_2,
      NULL param_3,
      NULL param_4,
      NULL param_5
