...
BEGIN
  ...
  IF :NEW.intervenant_id IS NOT NULL THEN
    OSE_AGREMENT.DEMANDE_CALCUL( :NEW.intervenant_id );
  END IF;
  
  IF :OLD.intervenant_id IS NOT NULL THEN
    OSE_AGREMENT.DEMANDE_CALCUL( :OLD.intervenant_id );
  END IF;
...
BEGIN
  ...
  FOR p IN (
    ...
    OSE_AGREMENT.DEMANDE_CALCUL( p.intervenant_id );
...
BEGIN
  ...
  IF :NEW.id IS NOT NULL THEN
    OSE_AGREMENT.DEMANDE_CALCUL( :NEW.id );
  END IF;
  
  IF :OLD.id IS NOT NULL THEN
    OSE_AGREMENT.DEMANDE_CALCUL( :OLD.id );
  END IF;
...
BEGIN
  OSE_AGREMENT.CALCULER_SUR_DEMANDE;
END;
