...
SELECT
  ...
  ose_divers.CALCUL_TAUX_FI( fi,fc,fa, 2) taux_fi,
  ose_divers.CALCUL_TAUX_FC( fi,fc,fa, 2) taux_fc,
  ose_divers.CALCUL_TAUX_FA( fi,fc,fa, 2) taux_fa,
  ose_divers.CALCUL_TAUX_FI( fi,fc,fa, 2)
+ ose_divers.CALCUL_TAUX_FC( fi,fc,fa, 2)
+ ose_divers.CALCUL_TAUX_FA( fi,fc,fa, 2) taux_total
FROM (
      select  0 fi, 50 fc,  0 fa,  1 ordre from dual
union select  0 fi,  0 fc, 50 fa,  2 ordre from dual
union select 50 fi,  0 fc,  0 fa,  3 ordre from dual
...
union select 50 fi,  0 fc, 50 fa,  7 ordre from dual
union select  0 fi, 50 fc, 50 fa,  8 ordre from dual
...
union select  1 fi,  1 fc,  1 fa, 38 ordre from dual
) tmp
order by ordre