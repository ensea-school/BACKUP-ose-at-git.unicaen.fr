CREATE OR REPLACE TRIGGER T_PAI_FOR_RES_SERVICE
AFTER INSERT 
OR UPDATE OF 
  formule_resultat_id,
	service_id
OR DELETE ON FORMULE_RESULTAT_SERVICE
FOR EACH ROW
BEGIN

  FOR p IN (
  
    SELECT DISTINCT
      fr.intervenant_id
    FROM
      formule_resultat fr
    WHERE
         fr.id = :NEW.formule_resultat_id
      OR fr.id = :OLD.formule_resultat_id
  
  ) LOOP
  
    OSE_PAIEMENT.DEMANDE_CALCUL( p.intervenant_id );
  
  END LOOP;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_FOR_RES_SERVICE_REF
AFTER INSERT 
OR UPDATE OF 
  formule_resultat_id,
	service_referentiel_id
OR DELETE ON FORMULE_RESULTAT_SERVICE_REF
FOR EACH ROW
BEGIN

  FOR p IN (
  
    SELECT DISTINCT
      fr.intervenant_id
    FROM
      formule_resultat fr
    WHERE
         fr.id = :NEW.formule_resultat_id
      OR fr.id = :OLD.formule_resultat_id
  
  ) LOOP
  
    OSE_PAIEMENT.DEMANDE_CALCUL( p.intervenant_id );
  
  END LOOP;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_FOR_RESULTAT
AFTER INSERT 
OR UPDATE OF 
  type_volume_horaire_id,
  etat_volume_horaire_id,
	intervenant_id
OR DELETE ON FORMULE_RESULTAT
FOR EACH ROW
BEGIN

  IF :NEW.intervenant_id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :NEW.intervenant_id );
  END IF;
  
  IF :OLD.intervenant_id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :OLD.intervenant_id );
  END IF;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_INTERVENANT
AFTER INSERT 
OR UPDATE OF 
  annee_id,
	structure_id
OR DELETE ON INTERVENANT
FOR EACH ROW
BEGIN

  IF :NEW.id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :NEW.id );
  END IF;

  IF :OLD.id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :OLD.id );
  END IF;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_SERVICE
AFTER INSERT 
OR UPDATE OF 
    element_pedagogique_id
OR DELETE ON SERVICE
FOR EACH ROW
BEGIN

  IF :NEW.intervenant_id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :NEW.intervenant_id );
  END IF;
  
  IF :OLD.intervenant_id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :OLD.intervenant_id );
  END IF;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_SERVICE_REFERENTIEL
AFTER INSERT 
OR UPDATE OF 
    structure_id
OR DELETE ON SERVICE_REFERENTIEL
FOR EACH ROW
BEGIN

  IF :NEW.intervenant_id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :NEW.intervenant_id );
  END IF;
  
  IF :OLD.intervenant_id IS NOT NULL THEN
    OSE_PAIEMENT.DEMANDE_CALCUL( :OLD.intervenant_id );
  END IF;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_ELEMENT_PEDAGOGIQUE
AFTER INSERT 
OR UPDATE OF 
  structure_id
OR DELETE ON ELEMENT_PEDAGOGIQUE
FOR EACH ROW
BEGIN

  FOR p IN (
  
    SELECT DISTINCT
      s.intervenant_id
    FROM
      service s
    WHERE
         s.element_pedagogique_id = :NEW.id
      OR s.element_pedagogique_id = :OLD.id
  
  ) LOOP
  
    OSE_PAIEMENT.DEMANDE_CALCUL( p.intervenant_id );
  
  END LOOP;

END;

/

CREATE OR REPLACE TRIGGER T_PAI_MISE_EN_PAIEMENT
AFTER INSERT 
OR UPDATE OF 
  periode_paiement_id,
	formule_res_service_id,
	formule_res_service_ref_id,
	histo_creation,
	histo_destruction
OR DELETE ON MISE_EN_PAIEMENT
FOR EACH ROW
BEGIN

  FOR p IN (
  
    SELECT DISTINCT
      fr.intervenant_id
    FROM
      formule_resultat fr
      LEFT JOIN formule_resultat_service frs ON frs.formule_resultat_id = fr.id
      LEFT JOIN formule_resultat_service_ref frsr ON frsr.formule_resultat_id = fr.id
    WHERE
         (frs.id  IS NOT NULL AND (frs.id  = :NEW.formule_res_service_id     OR frs.id  = :NEW.formule_res_service_id    ))
      OR (frsr.id IS NOT NULL AND (frsr.id = :NEW.formule_res_service_ref_id OR frsr.id = :NEW.formule_res_service_ref_id))
  
  ) LOOP
  
    OSE_PAIEMENT.DEMANDE_CALCUL( p.intervenant_id );
  
  END LOOP;

END;

/








CREATE OR REPLACE TRIGGER T_PAI_ELEMENT_PEDAGOGIQUE_S
AFTER INSERT OR UPDATE OR DELETE ON ELEMENT_PEDAGOGIQUE
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_FOR_RES_SERVICE_S
AFTER INSERT OR UPDATE OR DELETE ON FORMULE_RESULTAT_SERVICE
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_FOR_RES_SERVICE_REF_S
AFTER INSERT OR UPDATE OR DELETE ON FORMULE_RESULTAT_SERVICE_REF
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_FOR_RESULTAT_S
AFTER INSERT OR UPDATE OR DELETE ON FORMULE_RESULTAT
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_INTERVENANT_S
AFTER INSERT OR UPDATE OR DELETE ON INTERVENANT
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_MISE_EN_PAIEMENT_S
AFTER INSERT OR UPDATE OR DELETE ON MISE_EN_PAIEMENT
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_SERVICE_S
AFTER INSERT OR UPDATE OR DELETE ON SERVICE
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;

/


CREATE OR REPLACE TRIGGER T_PAI_SERVICE_REFERENTIEL_S
AFTER INSERT OR UPDATE OR DELETE ON SERVICE_REFERENTIEL
BEGIN
  OSE_PAIEMENT.CALCULER_SUR_DEMANDE;
END;
