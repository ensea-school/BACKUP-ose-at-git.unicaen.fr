...
class EtapeViewHelper extends AbstractViewHelper
{
    ...
    public function render()
    {
        ...
        if ($this->getView()->isAllowed($entity, Privileges::ODF_ETAPE_EDITION)) {
            $buttons .= '<a class="btn btn-default ajax-modal" href="' . $this->getView()->url('of/etape/modifier', ['etape' => $entity->getId()]) . '" data-event="etape-modifier"><i class="fa-solid fa-pencil"></i> Modifier</a>';
            $buttons .= '<a class="btn btn-default ajax-modal" href="' . $this->getView()->url('of/etape/supprimer', ['etape' => $entity->getId()]) . '" data-event="etape-supprimer"><i class="fa-solid fa-trash-can"></i Supprimer</a>';
        }
    ...
    {
        if (!$content) $content = '<i class="fa-solid fa-plus"></i Ajouter une formation';
