CREATE OR REPLACE TRIGGER T_TBL_VALID_REFERENTIEL
AFTER INSERT OR UPDATE OR DELETE ON TBL_VALIDATION_REFERENTIEL
FOR EACH ROW
BEGIN
  IF NOT UNICAEN_TBL.ACTIV_TRIGGERS THEN RETURN; END IF;
  
  IF :OLD.intervenant_id IS NOT NULL THEN
    UNICAEN_TBL.DEMANDE_CALCUL( 'workflow', UNICAEN_TBL.make_params('INTERVENANT_ID', :OLD.intervenant_id) );
  END IF;
  
  IF :NEW.intervenant_id IS NOT NULL THEN
    UNICAEN_TBL.DEMANDE_CALCUL( 'workflow', UNICAEN_TBL.make_params('INTERVENANT_ID', :NEW.intervenant_id) );
  END IF;
END;

/

CREATE OR REPLACE TRIGGER T_TBL_VALID_REFERENTIEL_S
AFTER INSERT OR UPDATE OR DELETE ON TBL_VALIDATION_REFERENTIEL
BEGIN
