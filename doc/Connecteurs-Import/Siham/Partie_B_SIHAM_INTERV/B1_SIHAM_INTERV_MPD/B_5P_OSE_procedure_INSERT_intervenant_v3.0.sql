...
-- ##A_PERSONNALISER_CHOIX_SIHAM## : si pas besoin supprimer V_BI_PASSEPORT et DBLink @BI et remonter directement le mail suivant vos règles
...
( 	----------------- SELECT DE V_INTERV -----------------------
    ...
    decode(v_aff_gene.qualit, '1', 1,2) 		as civilite_id
	...
	,v_aff_gene.uid_ldap						as uid_ldap						-- uid dans ldap pour @BI et verif si passeport univ valide
   ...
   ( select distinct b.nudoss
    ...
    from (select distinct
        ...
        ,case when trim(lieu.IDWKLO) = '0342321N' then ''
		...
		,case when trim(a.codtra) = 'TI050' and trim(lieu.IDWKLO) <> '0342321N' then 'O'
			else 'N'
        ...
        and ( 
                (	a.codtra like 'TI0%'  				-- ##A_PERSONNALISER_CHOIX_SIHAM## suivant codage TI Temps Incomplet
					...
					and lieu.idwklo not in ('0342321N') -- ##A_PERSONNALISER_CHOIX_SIHAM##
				...
				or (
					...
					-- rne lieu de travail
					and a.nudoss = lieu.nudoss
					and lieu.idwklo = '0342321N'    	-- ##A_PERSONNALISER_CHOIX_SIHAM##
					and trunc(lieu.dten00) >= p_d_deb_annee_univ
					and trunc(lieu.dtef00) <= p_d_fin_annee_univ
					and lieu.idwklo = rep2.cdcode
					and rep2.cdstco = 'DRE'
...
( -------------- BI (LDAP): recup si validation passeport informatique université
	...
	from BI_PERS.PERS_2_INTERMEDIAIRE@BI
...
BEGIN
	...
	FOR cur_pop in cur_ose_update_intervenant LOOP
		...
		for c1 in cur_intervenant(cur_pop.matcle, cur_pop.nudoss, p_d_deb_annee_univ, p_d_fin_annee_univ) loop
			...
			-------------
			v_ose_mail_pro := c1.mail_pro; 			-- ##A_PERSONNALISER_CHOIX_SIHAM## : si pas besoin supprimer V_BI_PASSEPORT et DBLink @BI et remonter directement le mail suivant vos règles
			if v_statut_new.code_type_intervenant = 'E' then   -- v3.0  07/01/2021	--- SI IE + passeport valide : mail pro sinon mail perso 
					if c1.um_passeport <> '1' then v_ose_mail_pro := c1.mail_perso; end if;
			end if;
			-------------
		  ...
		  ELSE
			...
			if v_maj_statut_a_faire in ('AUTO','OUI') then	-- v3.0
					...
					set					
						...
						,DATE_DEB_STATUT	= v_statut_new.date_deb_statut			-- v3.0  07/01/2021
