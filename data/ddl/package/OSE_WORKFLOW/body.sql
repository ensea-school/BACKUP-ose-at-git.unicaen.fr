CREATE
OR REPLACE PACKAGE BODY "OSE_WORKFLOW" AS
  TYPE t_dep_bloquante IS RECORD (
    id NUMERIC,
    to_delete BOOLEAN DEFAULT TRUE
  );
  TYPE
t_deps_bloquantes IS TABLE OF t_dep_bloquante INDEX BY PLS_INTEGER;
  TYPE
t_workflow_etape IS RECORD ( -- une étape d'un workflow
      id NUMERIC,
      to_delete BOOLEAN,
      to_update BOOLEAN,
      etape_id NUMERIC,
  ...
  TYPE
t_workflow IS TABLE OF t_workflow_etape INDEX BY VARCHAR2(20); -- une feuille de route
  TYPE
t_intervenant IS RECORD (
    annee_id NUMERIC,
  ...
  TYPE
t_intervenants IS TABLE OF t_intervenant INDEX BY PLS_INTEGER;      -- feuilles de routes de tous les intervenants concernés
  TYPE
t_etapes IS TABLE OF wf_etape%rowtype INDEX BY PLS_INTEGER;

  TYPE
t_dep IS TABLE OF wf_etape_dep%rowtype INDEX BY PLS_INTEGER;
  TYPE
t_deps IS TABLE OF t_dep INDEX BY PLS_INTEGER;
...
INTO eso
FROM wf_etape
WHERE id = etape_suiv_id;
SELECT ordre
INTO epo
FROM wf_etape
WHERE id = etape_prec_id;
...
BEGIN
    ...
    */
    ose_test.echo
('annee_id              = ' || intervenant.annee_id );
    ose_test.echo
('statut_id = ' || intervenant.statut_id );
    ose_test.echo
('type_intervenant_id   = ' || intervenant.type_intervenant_id );
    ose_test.echo
('type_intervenant_code = ' || intervenant.type_intervenant_code );
    ose_test.echo
('feuille_de_route      = [');

    e
:= intervenant.feuille_de_route.FIRST;
    LOOP
EXIT WHEN e IS NULL;
      ose_test.echo
('  index = ' || e );
      ose_test.echo
('    id              = ' || intervenant.feuille_de_route(e).id );
      IF
intervenant.feuille_de_route(e).etape_id IS NOT NULL THEN
...
END IF;
      ose_test.echo
('    structure_id    = ' || intervenant.feuille_de_route(e).structure_id );
      ose_test.echo
('    atteignable     = ' || intervenant.feuille_de_route(e).atteignable || ' (old=' || intervenant.feuille_de_route(e).old_atteignable || ')' );
      ose_test.echo
('    objectif        = ' || intervenant.feuille_de_route(e).objectif || ' (old=' || intervenant.feuille_de_route(e).old_objectif || ')'  );
      ose_test.echo
('    realisation     = ' || intervenant.feuille_de_route(e).realisation || ' (old=' || intervenant.feuille_de_route(e).old_realisation || ')'  );
      ose_test.echo
('    to_delete       = ' || CASE WHEN intervenant.feuille_de_route(e).to_delete THEN '1' ELSE '0' END );
      ose_test.echo
('    to_update       = ' || CASE WHEN intervenant.feuille_de_route(e).to_update THEN '1' ELSE '0' END );

      IF
intervenant.feuille_de_route(e).deps_bloquantes.COUNT > 0 THEN
        ose_test.echo('    dépendances bloquantes = [');
        b
:= intervenant.feuille_de_route(e).deps_bloquantes.FIRST;
        LOOP
EXIT WHEN b IS NULL;
          ose_test.echo
('        ' || b || ' => ');
          ose_test.echo
('            id        = ' || intervenant.feuille_de_route(e).deps_bloquantes(b).id        );
          ose_test.echo
('            to_delete = ' || CASE WHEN intervenant.feuille_de_route(e).deps_bloquantes(b).to_delete THEN '1' ELSE '0' END );
          ose_test.echo
('');

          b
:= intervenant.feuille_de_route(e).deps_bloquantes.NEXT(b);
END LOOP;
        ose_test.echo
('    ]');
END IF;
...
('');
      ...
      e
:= intervenant.feuille_de_route.NEXT(e);
END LOOP;
...
END;
  ...
  FUNCTION
MAKE_FR_ETAPE_INDEX( etape_id NUMERIC, structure_id NUMERIC DEFAULT NULL) RETURN VARCHAR2 IS
BEGIN
...
END;
  ...
  PROCEDURE ENREGISTRER( e IN OUT NOCOPY t_workflow_etape )
IS
    w tbl_workflow%rowtype;
    wed
wf_dep_bloquante%rowtype;
    b
NUMERIC;
BEGIN
    IF
...
:= intervenant.intervenant_id;
      w.statut_id
:= intervenant.statut_id;
      w.type_intervenant_id
:= intervenant.type_intervenant_id;
      w.type_intervenant_code
:= intervenant.type_intervenant_code;
      w.etape_id
:= e.etape_id;
      w.etape_code
:= etapes(e.etape_id).code;
      w.structure_id
:= e.structure_id;
      w.atteignable
:= e.atteignable;
...
ELSE
        w.id := e.id;
        IF
e.old_atteignable <> e.atteignable
...
END IF;
      ...
      b
:= e.deps_bloquantes.FIRST;
      LOOP
EXIT WHEN b IS NULL;
        IF
e.deps_bloquantes(b).to_delete THEN
DELETE
FROM wf_dep_bloquante
WHERE id = e.deps_bloquantes(b).id;
ELSIF
e.deps_bloquantes(b).id IS NULL THEN
...
END;
  ...
  FUNCTION
ETAPE_FRANCHIE( etape IN t_workflow_etape, need_done BOOLEAN DEFAULT FALSE ) RETURN FLOAT IS
    res FLOAT DEFAULT 0;
BEGIN
    IF
etape.objectif = 0 THEN
      IF need_done THEN RETURN 0;
ELSE RETURN 1;
END IF;
END IF;

    IF
etape.atteignable = 0 THEN RETURN 0;
END IF;
...
END;
  ...
  PROCEDURE CALCUL_ATTEIGNABLE( wf_etape IN OUT NOCOPY t_workflow_etape, d wf_etape_dep%rowtype )
IS
    workflow t_workflow;
    count_tested
...
PLS_INTEGER DEFAULT 0;
    p
VARCHAR2(20); -- index de l'étape précédente
BEGIN
    IF
d.type_intervenant_id IS NOT NULL AND d.type_intervenant_id <> intervenant.type_intervenant_id THEN
      RETURN; -- cette dépendance ne concerne pas notre intervenant
END IF;

    workflow
:= intervenant.feuille_de_route;
...
EXIT WHEN p IS NULL;
      IF
workflow(p).etape_id = d.etape_prec_id THEN
...
count_tested > 0 THEN
      ...
      IF d.integrale = 1 THEN
        ...
        IF count_na > 0 THEN
          ...
          wf_etape.deps_bloquantes
(d.id).to_delete := FALSE;
END IF;
...
IS
    ...
    e
VARCHAR2(20); -- index de l'étape courante
    d
PLS_INTEGER; -- ID de l'étape précédante
BEGIN
    e
:= intervenant.feuille_de_route.FIRST;
    LOOP
EXIT WHEN e IS NULL;
      workflow_etape
:= intervenant.feuille_de_route(e);

      IF
(NOT workflow_etape.to_delete) AND deps.exists(workflow_etape.etape_id) THEN -- s'il n'y a aucune dépendance alors pas de test!!
        d := deps(workflow_etape.etape_id).FIRST;
        LOOP
EXIT WHEN d IS NULL;

          CALCUL_ATTEIGNABLE
(intervenant.feuille_de_route(e), deps(workflow_etape.etape_id)(d));

          d
:= deps(workflow_etape.etape_id).NEXT(d);
END LOOP;
END IF;
      ENREGISTRER
(intervenant.feuille_de_route(e));
...
END;
  ...
  FUNCTION
MAKE_V_TBL_WORKFLOW(param VARCHAR2 DEFAULT NULL, VALUE VARCHAR2 DEFAULT NULL) RETURN CLOB IS
    p VARCHAR2(30);
    dems
CLOB;
    intervenant
        ...
        SELECT
          ...
          CASE
            WHEN e.code = ''DONNEES_PERSO_SAISIE'' THEN
              (d.completude_statut + d.completude_identite + d.completude_identite_comp + d.completude_contact + d.completude_adresse + d.completude_insee + d.completude_banque + d.completude_employeur + d.completude_autres) / 9
...
WHEN C%NOTFOUND;
      IF
NOT intervenants.exists(t.intervenant_id) THEN
        intervenants(t.intervenant_id).annee_id              := t.annee_id;
        intervenants
(t.intervenant_id).intervenant_id        := t.intervenant_id;
        intervenants
(t.intervenant_id).statut_id             := t.statut_id;
        intervenants
(t.intervenant_id).type_intervenant_id   := t.type_intervenant_id;
        intervenants
(t.intervenant_id).type_intervenant_code := t.type_intervenant_code;
END IF;
      we_ec
:= MAKE_FR_ETAPE_INDEX( t.etape_id, t.structure_id );
      we
:= new_we;
      we.id
:= t.id;
...
:= t.realisation;
      intervenants
(t.intervenant_id).feuille_de_route(we_ec) := we;
END LOOP;
CLOSE c;
...
ELSE
        intervenants(v.intervenant_id).annee_id              := v.annee_id;
        intervenants
(v.intervenant_id).intervenant_id        := v.intervenant_id;
        intervenants
(v.intervenant_id).statut_id             := v.statut_id;
        intervenants
(v.intervenant_id).type_intervenant_id   := v.type_intervenant_id;
        intervenants
(v.intervenant_id).type_intervenant_code := v.type_intervenant_code;
END IF;
      we_ec
:= MAKE_FR_ETAPE_INDEX( v.etape_id, v.structure_id );
      IF
intervenants(v.intervenant_id).feuille_de_route.exists(we_ec) THEN
...
:= v.realisation;
      intervenants
(v.intervenant_id).feuille_de_route(we_ec) := we;
END LOOP;
CLOSE c;
...
LOOP
FETCH C INTO wdb; EXIT
WHEN C%NOTFOUND;
      we_ec
:= MAKE_FR_ETAPE_INDEX( wdb.etape_id, wdb.structure_id );
      IF
intervenants(wdb.intervenant_id).feuille_de_route.exists(we_ec) THEN
...
:= wdb.structure_id;
        we.deps_bloquantes
(wdb.wf_etape_dep_id).id := wdb.id;
        intervenants
(wdb.intervenant_id).feuille_de_route(we_ec) := we;
END IF;
END LOOP;
CLOSE c;
i
