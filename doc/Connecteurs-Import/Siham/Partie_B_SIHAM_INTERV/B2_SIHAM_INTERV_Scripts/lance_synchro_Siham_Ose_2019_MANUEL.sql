...
v_annee_id              number(9)		:= 2019;  				-- ANNEE_ID de OSE : param manuellement et alim auto de v_d_deb_annee_univ v_d_fin_annee_univ
v_date_systeme          date			:= to_date('31/08/2020','DD/MM/YYYY');   	-- date d'observation (par défaut sysdate, mais on peut générer tous les ACTIFS pour une date passée)
																-- !!  Pour que les SYNCHRO fonctionnent IL FAUT QUE  v_d_deb_annee_univ <= v_date_systeme <= v_d_fin_annee_univ.

v_type_transfert        varchar2(10)    := 'MANUEL';          	-- Valeurs 'MANUEL'/'DIFF'/'ACTIFS'
v_date_profondeur		date			:= to_date('31/08/2020','DD/MM/YYYY'); 	-- dans le cas de DIFF : synchronise les dossiers modifiés dans Siham depuis cette date
