...
set trimspool on
set serveroutput on
spool /data/appli/ose/cron/log_siham/PREP2_trace_sql_synchro_Siham_Ose.lst append;
--spool C:\temp\Toad_exports\trace_sql_synchro_Siham_Ose.lst;
...
v_annee_id              number(9)		:= 2021;  				-- ANNEE_ID de OSE : param manuellement et alim auto de v_d_deb_annee_univ v_d_fin_annee_univ
v_date_systeme          date			:= to_date('01/09/'||to_char(v_annee_id),'DD/MM/YYYY');  	-- date d'observation (par défaut sysdate, mais on peut générer tous les ACTIFS pour une date passée)
																-- !!  Pour que les SYNCHRO fonctionnent IL FAUT QUE  v_d_deb_annee_univ <= v_date_systeme <= v_d_fin_annee_univ.

v_type_transfert        varchar2(10)    := 'ACTIFS';          	-- Valeurs 'MANUEL'/'DIFF'/'ACTIFS'
v_date_profondeur		date			:= to_date('01/09/'||to_char(v_annee_id),'DD/MM/YYYY');
--trunc(sysdate-2); 	-- dans le cas de DIFF : synchronise les dossiers modifiés dans Siham depuis cette date

/* ------------------------------------------------------------------------------------------
!! BIEN REGLER LES VARIABLES CI-DESSUS : cf. détails Doc MOP_SYNCHRO_OSE_RH_v1.3_L.docx

	MANU : 	Mode manuel = que les dossiers à forcer insérés manuellement dans UM_TRANSFERT_FORCE (ne tient pas compte de v_date_profondeur)
