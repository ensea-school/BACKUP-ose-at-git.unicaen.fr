...
Le mode installation d'OSE liste toutes les dépendances nécessaires et teste leur présence sur votre serveur.
...
Spécifique à Debian (il en existe un paquet intégré à Debian) :
Attention également : sous Debian, unoconv peut s'installer par `apt-get`, mais il faut aussi installer LibreOffice
Writer, la dépendance n'étant pas automatique (`apt-get install unoconv libreoffice-writer`).
...
Cela se fait au moyen de la commande suivante (depuis le répertoire d'OSE) :
...
Dans tous les cas, c'est le script bash d'OSE qui sera appelé.
Le script est situé dans le répertoire d'OSE, `bin/ose`.
...
```bash
/[votre chemin ose]/bin/ose synchronisation synchro
```
...
0    5-17 *   *   1-5 /[votre chemin ose]/ose/bin/ose notifier-indicateurs  > /tmp/oselog 2>&1
0    7-21 *   *   1-6 /[votre chemin ose]/bin/ose synchronisation synchro   > /tmp/oselog 2>&1
0      20 *   *   1-6 /[votre chemin ose]/bin/ose chargens-calcul-effectifs > /tmp/oselog 2>&1
0    6,14 *   *   1-6 /[votre chemin ose]/bin/ose calcul-tableaux-bord      > /tmp/oselog 2>&1
0       3 *   *   1,4 /[votre chemin ose]/bin/ose formule-calcul            > /tmp/oselog 2>&1
0       7 15  12    * /[votre chemin ose]/bin/ose maj-taux-mixite           > /tmp/oselog 2>&1
0       4 *   *   1-6 /[votre chemin ose]/bin/ose maj-exports               > /tmp/oselog 2>&1
```
...
Le mieux est, dans ce contexte, de réaliser ces synchronisations manuellement, table par table, depuis
l'interface d'OSE.
Cela vous permettra de :

* mieux maitriser les flux de données entre votre SI et OSE ;
* mieux identifier des problèmes éventuels au niveau des connecteurs puisque OSE affiche les erreurs rencontrées à
  l'écran ;
* d'avoir une offre de formation de test "stable", c'est-à-dire qui ne se met pas à jour toute seule, ce qui simplifiera
  vos tests.

En revanche, il est vivement recommandé de bien paramétrer en préproduction les autres tâches
(calcul des effectifs Chargens, calcul des tableaux de bord, formule de calcul).
...
[Le guide administratif](https://redmine.unicaen.fr/Etablissement/projects/deploiement_ose/dmsf?folder_id=3204) vous
aidera en cela.
